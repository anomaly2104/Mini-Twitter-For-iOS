...
- (IBAction)postTweet:(id)sender {
    ...
    APICompletionBlock postTweetCompletionBlock = ^(NSDictionary *responseData) {
        ...
        [MTTweet tweetWithTwitterData:responseData
               inManagedObjectContext:tabBarController.currentUser.managedObjectContext];
        [self tweetSuccess];
    };
    NSString* tweet = self.tweetMessageTextBox.text;
    [self.tweeterFetcher postTweet:tweet
                   completionBlock:postTweetCompletionBlock
